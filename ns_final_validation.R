...
p3<-predict(rf_nonspatial_alldata, till_df, predict.all=TRUE) 
...
ind3$rms<- lapply(ind3$observed, ind3$predicted.aggregate, FUN= RMSE)
...
png( filename = "cal_asp_scattercode.png", width = 3.25, height = 2.5, units = "in", res = 200)
par(mar= c(3, 3, 0, 1))
...
png( filename = "D:/GIS_SCOUT/GIS_SCOUT/figures/cal_asp_bins.png", width = 3.25, height = 1.25, units = "in", res = 200)
par(mar= c(0, 3, 1, 1))
